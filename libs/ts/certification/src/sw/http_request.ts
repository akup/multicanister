...
// @ts-expect-error
const global = (globalThis as any).global = globalThis;
 ...
 const shouldFetchRootKey: boolean = false;
 ...
 /**
	...
	*/
 export function splitHostnameForCanisterId(hostname: string): [Principal, string] | null {
 ...
 export function maybeResolveCanisterIdFromHttpRequest(request: Request, isLocal: boolean) {
	 return (
 ...
 export const canisterIdlFactory: IDL.InterfaceFactory = ({ IDL }) => {
 ...
 export const makeChecks = async (request: Request, maybeCanisterId: Principal, isLocal: boolean = false) => {
	const url = new URL(request.url);
