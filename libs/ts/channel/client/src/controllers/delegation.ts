...
import {DelegationChain, Ed25519KeyIdentity, Delegation, DelegationIdentity} from '@dfinity/identity';
...
export class DelegationController implements Controller {
  ...
  agentController: AgentController;
	...
	constructor(public gateway: Channel<any, any>, opts: DelegationControllerProps) {
    this.opts = opts;
    this.agentController = opts.agent;
  }

	public requestAuth = async (params: ClientParams = {target: 'top'}) => {
		const payload: SetIdentityPayload = await this.gateway.request('requestIdentity', {canisterId: this.opts.canisterId}, params);
  ...
  private constructIdentity = (payload: SetIdentityPayload) => {
    ...
    const delegations = payload.delegations.map(d => ({
      delegation: new Delegation(blobFromUint8Array(d.delegation.pubkey), d.delegation.expiration, d.delegation.targets),
      signature: blobFromUint8Array(d.signature),
    }));
    const delegationChain = DelegationChain.fromDelegations(delegations, derBlobFromBlob(blobFromUint8Array(payload.userPublicKey)));
