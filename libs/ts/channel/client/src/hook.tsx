...
export function ProvideAuth<C extends string = any, S extends string = any>({ channel, children }: {channel: ChannelConsumer<C, S>, children: any}) {
  const auth = useProvideAuth(channel);
  return <authContext.Provider value={auth}>{children}</authContext.Provider>;
}

export function useProvideAuth<C extends string = any, S extends string = any>(channel: ChannelConsumer<C, S>) {
