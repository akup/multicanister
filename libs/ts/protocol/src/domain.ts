...
export const isFact = (p: any): p is Fact => {
	const base = !!p && 'documentId' in p && typeof p.documentId == 'string' && 'values' in p && Array.isArray(p.values);
	if (!base) {
		return false;
	}
	...
	return (
		'alias' in e && typeof e.alias == 'string' && e.alias
		&& 'value' in e && Array.isArray(e.value)
		&& 'nonce' in e && Array.isArray(e.nonce) && !!e.nonce.length
	);
}
