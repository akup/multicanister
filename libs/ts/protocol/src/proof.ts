...
export const merkalizeValues = (values: FactDataValue<Value>[], schema: TemplateSchema): number[] => {
	return merkalize(values.map(v => ({...v, value: serialize(v.value)})), schema);
};
...
export const generateProof = (reveal_fields: bigint[], values: FactDataValue<number[]>[], schema: TemplateSchema): Proof => {
